...
export const hardTicketSummary = (values: Product, currency?: string): RowSummary => {
    if (!values.is_hard_ticket) {
        return {text: t`Digital ticket only`, emphasized: false};
    }

    const fee = Number(values.hard_ticket_fee || 0);

    if (fee > 0) {
        return {text: t`Mailed ticket · ${formatCurrency(fee, currency)} fee`, emphasized: true};
    }

    return {text: t`Mailed ticket · No extra fee`, emphasized: true};
};
